...
def recepcionRobots(ch, method, properties, body):
    ...
    if (probability >= ceiling):
        time.sleep(wait)
        #modificar status del pedido
        connection = pika.BlockingConnection(pika.ConnectionParameters(host='localhost'))
        channel = connection.channel()
        channel.queue_declare(queue='controlador')
        channel.basic_publish(exchange='', routing_key='controlador', body = body)
        connection.close()
    else:
...
def parseBody(body:str):
    if body.find('Cliente:',0,8):
        print("Client signup efectivo {}".format(body))
        return ""
    elif body.find('Pedido:',0,8):
        print("Iniciar busqueda de pedido: {}".format(body))
        for ele in body.split(";",3):
            if ele.find("id_pedido"):
                for value in ele.split("="):
                    if value == " ":
                        return "error"
                else:   
                    return "robot"
    elif body.find('Robot:',0,8):
...
def on_request(ch, method, props, body):
    ...
    if verif == "":
        ...
        ch.basic_publish(exchange='',
                        ...
                        properties=pika.BasicProperties(correlation_id = \
                                                            props.correlation_id),
                        body="Error debes hacer signup primero")
        ch.basic_ack(delivery_tag=method.delivery_tag)
    """
    else:
